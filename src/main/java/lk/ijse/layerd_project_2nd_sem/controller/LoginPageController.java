...
public class LoginPageController {
    ...
    public void btnSignInOnAction(ActionEvent actionEvent) {
        ...
        try {
            ResultSet resultSet = SQLUtil.executeQuery("SELECT * FROM user WHERE user_name = ? AND password = ?",
                    inputUsername, inputPassword
            );
            if (resultSet.next()) {
                try {
                    Parent dashboardRoot = FXMLLoader.load(getClass().getResource("/DashboardPage.fxml"));
    ...
    public void btnSignUpOnAction(ActionEvent actionEvent) {
        try {
            Parent dashBoardRoot = FXMLLoader.load(getClass().getResource("/SignUpPage.fxml"));
            Stage dashBoardStage = (Stage) ((Node) actionEvent.getSource()).getScene().getWindow();
            dashBoardStage.setScene(new Scene(dashBoardRoot));
            dashBoardStage.setTitle("Alpha Modifications");
